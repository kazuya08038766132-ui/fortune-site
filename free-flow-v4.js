(() => {
"use strict";
const BUILD="SPEC195-BUILD-37",$=id=>document.getElementById(id);
...
async function run(){
 ...
 const palmQuality=await window.PalmPublicQuality.quality(palm);if(palmQuality.status!=="ACCEPT"){const msg=window.PalmGateCopyV1?.message?.(palmQuality.reason)||"写真を確認できませんでした。別の写真を選んでください。";show(`<div class="captureError"><h3>手相写真を確認してください</h3><p>${esc(msg)}</p><p class="small">判定理由: ${esc(palmQuality.reason||"UNKNOWN")}</p></div>`);setBtn("無料鑑定を試す");return}
